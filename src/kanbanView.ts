...
export class KanbanView extends BasesView {
	...
	private _prefs: {
		...
		hiddenColumns: new Set(), // columnValue (per group property)
	};
	...
	private loadConfig(): void {
		...
		const rawTemplate = this.config?.get('templateFile');
		this.templateFile = typeof rawTemplate === 'string' && rawTemplate.trim() ? rawTemplate.trim() : null;
	...
	private _loadPrefs(propertyId: BasesPropertyId, swimlanePropertyId: BasesPropertyId | null): void {
		...
		// Hidden columns — per group property. Same shape as columnOrders.
	...
	private render(): void {
		try {
			// Apply the configurable board height cap (matters most for kanban
			// embedded in notes, where the container has no intrinsic height).
			if (this.boardMaxHeight) {
				this.containerEl.style.setProperty('--obk-board-max-height', `${this.boardMaxHeight}px`);
			} else {
			...
			// Columns the user has hidden are kept in the order/prefs but not rendered.
			const visibleOrderedValues = orderedValues.filter((v) => !this._prefs.hiddenColumns.has(v));
			...
			if (!existingBoard || modeChanged || groupChanged || optionsChanged) {
				this.fullRebuild(visibleOrderedValues, lanes, hasSwimlanes);
			} else {
				this.patchBoard(visibleOrderedValues, lanes, hasSwimlanes);
			}
	...
	/**
	 * Renders (or refreshes) the bar above the board listing hidden columns as
	 * chips. Clicking a chip unhides that column. The bar is the container's
	 * first child so it sits above the board; it is removed when nothing is hidden.
	 */
	private renderHiddenBar(allColumnValues: string[]): void {
		const existing = this.containerEl.querySelector<HTMLElement>(`.${CSS_CLASSES.HIDDEN_BAR}`);
		const hidden = allColumnValues.filter((v) => this._prefs.hiddenColumns.has(v));
		...
		bar.className = CSS_CLASSES.HIDDEN_BAR;
		bar.empty();
		if (!existing) this.containerEl.prepend(bar);

		const label = bar.createSpan({ cls: CSS_CLASSES.HIDDEN_BAR_LABEL });
		setIcon(label.createSpan(), 'eye-off');
		label.createSpan({ text: ' Hidden:' });

		for (const value of hidden) {
			const chip = bar.createDiv({ cls: CSS_CLASSES.HIDDEN_CHIP });
			chip.setAttribute('role', 'button');
			chip.setAttribute('tabindex', '0');
			chip.setAttribute('aria-label', `Show column: ${value}`);
			chip.createSpan({ text: value });
			setIcon(chip.createSpan(), 'plus');
			const show = () => this.unhideColumn(value);
	...
	private unhideColumn(value: string): void {
		if (!this._prefsPropertyId) return;
	...
	private deleteCard(file: TFile): void {
		if (!this.app) return;
		new ConfirmModal(this.app, {
			title: 'Delete note',
			message: `Move "${file.basename}" to trash? This deletes the whole note.`,
			confirmText: 'Delete',
			onConfirm: async () => {
				try {
					await this.app.fileManager.trashFile(file);
				} catch (error) {
					console.error('KanbanView: error deleting file', error);
	...
	private handleSwimlaneColumnDrop(evt: Sortable.SortableEvent): void {
		...
		// The DOM only contains visible columns; preserve hidden ones so they
		// aren't dropped from the saved order.
		const hidden = this._prefs.columnOrder.filter((v) => !order.includes(v));
		this._prefs.columnOrder = [...order, ...hidden];
		this._persistPrefs();
		this.render();
	}
	...
	private _buildCardCallbacks(): CardCallbacks {
		return {
			...
			onDeleteCard: (file) => this.deleteCard(file),
		};
	}
	...
	static getViewOptions(this: void): ViewOption[] {
		return [
			...
			},
			{
				...
				type: 'file',
				key: 'templateFile',
				placeholder: 'Optional: template for + button',
				filter: (file) => file.extension === 'md',
			},
			{
				displayName: 'Board max height (px)',
				type: 'slider',
				key: 'boardMaxHeight',
				default: 0,
				min: 0,
