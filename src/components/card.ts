...
import { Keymap, NullValue, setIcon } from 'obsidian';
...
export function createCard(entry: BasesEntry, ctx: CardRenderCtx, cb: CardCallbacks): HTMLElement {
	...
	// Delete button — absolutely positioned, revealed on card hover via CSS.
	const deleteBtn = cardEl.createDiv({ cls: CSS_CLASSES.CARD_DELETE_BTN });
	deleteBtn.setAttribute('aria-label', `Delete ${entry.file.basename}`);
	deleteBtn.setAttribute('role', 'button');
	deleteBtn.setAttribute('tabindex', '0');
	setIcon(deleteBtn, 'trash-2');
	const triggerDelete = (e: Event) => {
		e.preventDefault();
		e.stopPropagation();
		cb.onDeleteCard(entry.file);
	};
	deleteBtn.addEventListener('click', triggerDelete);
	deleteBtn.addEventListener('auxclick', (e) => e.stopPropagation());
	deleteBtn.addEventListener('mousedown', (e) => e.stopPropagation());
	deleteBtn.addEventListener('keydown', (e) => {
		if (e.key === 'Enter' || e.key === ' ') triggerDelete(e);
	});
